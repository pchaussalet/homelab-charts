...
helm repo update
helm install frigate geek-cookbook/frigate
```
...
helm install frigate \
  --set env.TZ="America/New York" \
    geek-cookbook/frigate
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install frigate geek-cookbook/frigate -f values.yaml
```
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/geek-cookbook/frigate?modal=changelog)
