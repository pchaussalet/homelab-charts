...
helm repo update
helm install haste-server geek-cookbook/haste-server
```
...
helm install haste-server \
  --set env.TZ="America/New York" \
    geek-cookbook/haste-server
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install haste-server geek-cookbook/haste-server -f values.yaml
```
...
| image.repository | string | `"ghcr.io/geek-cookbook/haste-server"` | image repository |
...
A historical overview of changes can be found on [ArtifactHUB](https://artifacthub.io/packages/helm/geek-cookbook/haste-server?modal=changelog)
